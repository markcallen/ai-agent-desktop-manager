...
function throwOnFailure(op: string, unit: string, code: number, stderr: string, stdout: string) {
  if (code === 0) return;
...
export async function systemctlStart(unit: string) {
  const res = await execCmd(config.systemctlBin, ["start", unit], { sudo: true });
  throwOnFailure("systemctl_start", unit, res.code, res.stderr, res.stdout);
  return res;
}

export async function systemctlStop(unit: string) {
  const res = await execCmd(config.systemctlBin, ["stop", unit], { sudo: true });
