...
async function importServerWithEnv() {
  ...
  execMod.setExecRunner(async (cmd, args, opts) => {
    ...
    if (cmd.endsWith("nginx") && args[0] === "-t") {
      if (failNginxTest) return { code: 1, stdout: "", stderr: "invalid nginx config" };
      return { code: 0, stdout: "ok", stderr: "" };
    }

    if (cmd.endsWith("systemctl") && args[0] === "reload" && args[1] === "nginx") {
    ...
    if (cmd.endsWith("systemctl") && (args[0] === "start" || args[0] === "stop")) {
      return { code: 0, stdout: "", stderr: "" };
    }
...
test("auth token is required when configured", async () => {
  ...
  const withAuth = await app.inject({ method: "GET", url: "/health", headers: authHeaders });
...
test("create failure on nginx test rolls back units, snippet, and state record", async () => {
  ...
  const list = await app.inject({ method: "GET", url: "/v1/desktops", headers: authHeaders });
  assert.equal(list.statusCode, 200);
  assert.equal(list.json().desktops.length, 1, "only initial successful desktop should remain");

  const snippetPath = path.join(tmpRoot, "nginx", "desk-2.conf");
  await assert.rejects(fs.access(snippetPath));

  const stopCalls = calls.filter((c) => c.cmd.endsWith("systemctl") && c.args[0] === "stop");
